jQuery(function ($) {
    ...
    WooCommerceNotices.prototype.initNotices = function () {
        var self = this;
        $(document.body).on('click', '.woocommerce-message .close-notice, .woocommerce-notice .close-notice, .woocommerce-error .close-notice', function () {
            self.hideNotice($(this).parent());
        });
        $(document.body).on('mouseenter', '.woocommerce-message.delayed, .woocommerce-notice.delayed, .woocommerce-error.delayed', function () {
            self.removeDelay($(this));
        });
        $(document.body).on('mouseleave', '.woocommerce-message.delayed, .woocommerce-notice.delayed, .woocommerce-error.delayed', function () {
            self.addDelay($(this));
        });
        $(document.body).on('notifications_loaded wc_cart_emptied updated_wc_div', function () {
            self.addDelay();
        });
        $(document.body).find('.woocommerce-notices-wrapper .woocommerce-message, .woocommerce-notices-wrapper .woocommerce-error').each(function (index, el) {
            self.addDelay($(el));
        })
